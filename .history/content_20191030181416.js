    ...
    for (var i = 0; i < text.length; i++) {
        for (var k = 0; k < nl.length; k++) {
            ...
            if (s == 0 || (s > 0 && !isAlphaNumeric(text[i].textContent[s - 1]))) {
                ...
                var temp = text[i].textContent.toLowerCase().substring(e).match(/ '".!?/);
                if (temp != null) {
                    e = temp.index;
                    window.alert(text[i].textContent.substring(s, e))
                }

                temp_string = text[i].textContent.substring(0, s) + tl[k];
                if (e != -1)
                    temp_string += text[i].textContent.substring(e)
