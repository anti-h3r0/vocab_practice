...
function subWord(nl, tl, text, i, k, s, e) {
        if (nl[k] == "i")
        console.log(nl[k] + " - " + text[i].textContent[e] + ":");
            ...
            for (var k = 0; k < nl.length; k++) {
                ...
                if (s == 0 || (s > 0 && !isAlphaNumeric(text[i].textContent[s - 1]) {
                    if (e === text[i].textContent.length) {
                        if (nl[k] == "i")
                            console.log("'i' located at the end of the string");
                        subWord(nl, tl, text, i, k, s, e);
                    }
                    else if (e < text[i].textContent.length) {
                        if (nl[k] == "i") {
                            console.log("'i' is not located at the end of the string");
                            console.log(text[i].textContent);
                        }

                         if(!isAlphaNumeric(text[i].textContent[e])) {
                            if (nl[k] == "i") 
                                console.log("'e' is not located at the end of the string");
                            subWord(nl, tl, text, i, k, s, e);
                        }
                    }
