...
function subWord(nl, tl, text, i, k, s, e) {
...
function alterText(nl, tl) {
    ...
    if (text.length > 0) {
        for (var i = 0; i < 20/*text.length*/; i++) {
            ...
            var found = text[i].textContent.replace('\u2019', "\'");
            found = found.replace('\u201D', "\"");
            found = text[i].textContent.replace('\u201C', "\"");
            if (found != -1) {
                window.alert(found + " : " + text[i].textContent);
                // for (var k = 0; k < found; k++){
                //     console.log(text[i].textContent.charCodeAt(k) + " : " + text[i].textContent[k]);
                // }
            }
            ...
            //     if (s == 0 || (s > 0 && !isAlphaNumeric(text[i].textContent[s - 1]))) {
            //         if (e === text[i].textContent.length) {
            //             subWord(nl, tl, text, i, k, s, e);
            //         }
            //         else if (e < text[i].textContent.length) {
            //             if (!isAlphaNumeric(text[i].textContent[e])) {
            //                 if (text[i].textContent[e] == "\&quotes") { }
